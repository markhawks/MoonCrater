...
for command in php psql grep; do command -v "$command" >/dev/null || { echo "Missing command: $command" >&2; exit 1; }; done
php -m | grep -q '^pdo_pgsql$' || { echo 'Missing PHP extension: pdo_pgsql' >&2; exit 1; }
for file in public/*.php app/*.php app/Domain/*.php app/View/*.php bin/*.php; do php -l "$file" >/dev/null; done
bash -n setup/*.sh setup/rhel10/*.sh tests/*.sh
