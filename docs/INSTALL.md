...
in addition to deploying MoonCrater.

1. Install Apache, PHP with `pdo_pgsql`, PostgreSQL and `psql`.
2. Place the repository in `/opt/mooncrater` or run `sudo setup/install.sh`.
...
7. Install `setup/apache/mooncrater.conf.example`, changing `ServerName` if necessary.
...
example uses a root-owned `/etc/mooncrater/apache-env.conf`, which should be mode `0640` or more
